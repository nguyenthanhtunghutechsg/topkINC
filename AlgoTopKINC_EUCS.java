...
public class AlgoTopKINC_EUCS {
    ...
    public void runAlgorithm(String input, String output, int k, int firstLine, int lastLine, boolean EUCS) throws IOException {
                ...
                if (tid >= firstLine) {
                    ...
                    for (int i = 0; i < transactionItems.size(); i++) {
                        ...
                        for (int j = i + 1; j < transactionItems.size(); j++) {
                            Item itemObjJ = transactionItems.get(j);
        ...
        if (firstTime) {
            raisingThresholdUtilityList();
            raisingThresholdEUCS();
            List<Integer> listAllItem = new ArrayList<>();
            for (UtilityList ul : mapItemToUtilityList.values()) {
                if (mapItemToTWU.get(ul.item) >= min_utility) {
                    listAllItem.add(ul.item);
                }
            }
            ...
            raisingThresholdByLeaf(listAllItem);
    ...
    public void raisingThresholdEUCS() {
        long value = 0L;
        for (Entry<Integer, Map<Integer, PairItem>> entry : mapEUCS.entrySet()) {
            for (Entry<Integer, PairItem> entry2 : entry.getValue().entrySet()) {
    ...
    public void raisingThresholdByLeaf(List<Integer> AllItemToUtilityList) {//all sort item in UL
        long LIU = 0L;
        // LIU-Exact
    ...
    void raisingThresholdUtilityList() {
        for (Entry<Integer, UtilityList> entry : mapItemToUtilityList.entrySet()) {
     ...
     * @param prefixLength an item to be appended to the prefix
